...
def get_ppo_sensitivity_score(query: str) -> float:
    ...
    # Broadened sensitivity keywords to cover all 25 domains from prompts_data.py
    # High sensitivity keywords (personal, specific, sensitive topics, proprietary, confidential)
...
def generate_user_specific_queries(user_id: str, num_prompts_per_user: int, domain_mix_ratio: dict) -> list:
    """
    ...
    normalized_mix = {k: v / sum(domain_mix_ratio.values()) for k, v in domain_mix_ratio.items()}
    ...
    for _ in range(num_prompts_per_user):
        ...
        # Fallback if domain pool exhausted, try from any pool
        if not available_for_domain:
            ...
            if not all_remaining_prompts:
                # If truly all prompts exhausted, re-use existing ones to meet count
...
if __name__ == "__main__":
    NUM_TRIALS = 30 # Number of independent trials for statistical significance
    N_USERS_PER_SCENARIO = 100 # Number of unique users per scenario in each trial
    N_PROMPTS_PER_USER = 10 # Number of prompts each user submits
    ...
    print(f"Total queries per trial: {N_USERS_PER_SCENARIO * N_PROMPTS_PER_USER}")
    print(f"Total queries across all trials: {NUM_TRIALS * N_USERS_PER_SCENARIO * N_PROMPTS_PER_USER}")


    for trial in range(1, NUM_TRIALS + 1):
        print(f"\n--- Running Trial {trial}/{NUM_TRIALS} ---")

        # Create unique users for THIS trial's scenarios
        # We need to create a *fresh set* of N_USERS_PER_SCENARIO clients for EACH trial
        # to ensure independence of trials for statistical tests.
        # Each client gets a unique ID and a varied privacy preference.
        
        # --- Create Users for Current Trial (and their query sets) ---
        trial_clients_and_queries = [] # List of (Client_instance, list_of_queries_for_this_client)
        for i in range(N_USERS_PER_SCENARIO):
            user_id_base = f"user_{i:03d}_T{trial}"
            # Assign varied privacy preferences
            pref = random.choice(["High", "Medium", "Low"])
            max_eps = {"High": 0.5, "Medium": 1.5, "Low": 5.0}[pref] 
            
            client_instance = Client(user_id=user_id_base, privacy_preference=pref, epsilon_max_user=max_eps)
            
            # Generate prompts specific to this user based on a domain mix.
            # You can customize the domain_mix_ratio for specific types of users
            # For this comprehensive run, we'll use a balanced default mix.
            user_queries = generate_user_specific_queries(client_instance.user_config.user_id, N_PROMPTS_PER_USER, DEFAULT_DOMAIN_MIX)
            
            trial_clients_and_queries.append((client_instance, user_queries))


        # --- Run each scenario for the CURRENT TRIAL'S set of users ---
        # Each scenario uses the same set of N_USERS_PER_SCENARIO clients and their generated queries.

        # Scenario 1: AEPB Dynamic (Your system) - High Privacy User Mix
        print(f"\n-- Scenario: {SCENARIO_AEPB_DYNAMIC_HIGH_PRIVACY} --")
        # Filter clients who have High privacy preference
        clients_for_this_scenario = [cq[0] for cq in trial_clients_and_queries if cq[0].user_config.privacy_preference == "High"]
        queries_for_this_scenario = [cq[1] for cq in trial_clients_and_queries if cq[0].user_config.privacy_preference == "High"]
        
        # If no High privacy users were randomly picked, add a fallback client for the scenario
        if not clients_for_this_scenario:
            fallback_client = Client(user_id=f"user_AEPB_HighP_Fallback_T{trial}", privacy_preference="High", epsilon_max_user=0.5)
            clients_for_this_scenario.append(fallback_client)
            queries_for_this_scenario.append(generate_user_specific_queries(fallback_client.user_config.user_id, N_PROMPTS_PER_USER, DEFAULT_DOMAIN_MIX))

        for client_idx, client_instance in enumerate(clients_for_this_scenario):
            run_simulation_scenario(client_instance, queries_for_this_scenario[client_idx], SCENARIO_AEPB_DYNAMIC_HIGH_PRIVACY, trial)


        # Scenario 2: AEPB Dynamic (Your system) - Medium Privacy User Mix
        print(f"\n-- Scenario: {SCENARIO_AEPB_DYNAMIC_MEDIUM_PRIVACY} --")
        clients_for_this_scenario = [cq[0] for cq in trial_clients_and_queries if cq[0].user_config.privacy_preference == "Medium"]
        queries_for_this_scenario = [cq[1] for cq in trial_clients_and_queries if cq[0].user_config.privacy_preference == "Medium"]
        if not clients_for_this_scenario: # Fallback
            fallback_client = Client(user_id=f"user_AEPB_MedP_Fallback_T{trial}", privacy_preference="Medium", epsilon_max_user=1.5)
            clients_for_this_scenario.append(fallback_client)
            queries_for_this_scenario.append(generate_user_specific_queries(fallback_client.user_config.user_id, N_PROMPTS_PER_USER, DEFAULT_DOMAIN_MIX))

        for client_idx, client_instance in enumerate(clients_for_this_scenario):
            run_simulation_scenario(client_instance, queries_for_this_scenario[client_idx], SCENARIO_AEPB_DYNAMIC_MEDIUM_PRIVACY, trial)


        # Scenario 3: Baseline - Always Delegate (No Local First)
        print(f"\n-- Scenario: {SCENARIO_BASELINE_ALWAYS_DELEGATE} --")
        for client_idx, client_instance in enumerate(trial_clients_and_queries): # Apply to all users for comparison
            run_simulation_scenario(client_instance[0], client_instance[1], SCENARIO_BASELINE_ALWAYS_DELEGATE, trial)


        # Scenario 4: Baseline - Fixed Epsilon (e.g., epsilon=1.0)
        print(f"\n-- Scenario: {SCENARIO_BASELINE_FIXED_EPSILON_1_0} --")
        for client_idx, client_instance in enumerate(trial_clients_and_queries): # Apply to all users
            run_simulation_scenario(client_instance[0], client_instance[1], SCENARIO_BASELINE_FIXED_EPSILON_1_0, trial, fixed_epsilon_value=1.0)
        
        # Scenario 5: Baseline - Fixed Epsilon (e.g., epsilon=0.1 - High Privacy)
        print(f"\n-- Scenario: {SCENARIO_BASELINE_FIXED_EPSILON_0_1} --")
        for client_idx, client_instance in enumerate(trial_clients_and_queries): # Apply to all users
            run_simulation_scenario(client_instance[0], client_instance[1], SCENARIO_BASELINE_FIXED_EPSILON_0_1, trial, fixed_epsilon_value=0.1)

        # Scenario 6: Baseline - Fixed Epsilon (e.g., epsilon=5.0 - Low Privacy)
        print(f"\n-- Scenario: {SCENARIO_BASELINE_FIXED_EPSILON_5_0} --")
        for client_idx, client_instance in enumerate(trial_clients_and_queries): # Apply to all users
            run_simulation_scenario(client_instance[0], client_instance[1], SCENARIO_BASELINE_FIXED_EPSILON_5_0, trial, fixed_epsilon_value=5.0)

        # Scenario 7: Baseline - Always Local (Never Delegates)
        print(f"\n-- Scenario: {SCENARIO_BASELINE_ALWAYS_LOCAL} --")
        for client_idx, client_instance in enumerate(trial_clients_and_queries): # Apply to all users
            run_simulation_scenario(client_instance[0], client_instance[1], SCENARIO_BASELINE_ALWAYS_LOCAL, trial)
    ...
    # Group by scenario_mode and trial_num, then take the mean cost for each trial
    ...
    cost_data_for_anova = {}
    for scenario_mode in avg_delegated_cost_per_trial_and_scenario['scenario_mode'].unique():
        costs = avg_delegated_cost_per_trial_and_scenario[avg_delegated_cost_per_trial_and_scenario['scenario_mode'] == scenario_mode]['simulated_total_cost'].tolist()
        if costs: # Only add if there's data (some scenarios might have 0 delegated queries in a trial)
    ...
    # For Chi-squared, we need total counts of delegated vs. not delegated across ALL queries and trials for the comparison scenarios
    
    # Get total counts for AEPB_Dynamic_High_Privacy
    aepb_dynamic_high_df = results_df[results_df['scenario_mode'] == SCENARIO_AEPB_DYNAMIC_HIGH_PRIVACY]
    aepb_dynamic_high_delegated = aepb_dynamic_high_df['is_delegated'].sum() # True counts as 1
    aepb_dynamic_high_local = len(aepb_dynamic_high_df) - aepb_dynamic_high_delegated
    
    # Get total counts for Baseline_AlwaysDelegate
