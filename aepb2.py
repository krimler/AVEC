...
def run_simulation_scenario(client_instance: Client, queries: list, scenario_name: str, trial_num: int, fixed_epsilon_value: float = None):
    ...
    # print(f"\n===== Running Scenario: {scenario_name}, Trial {trial_num} =====")
    ...
    for i, query in enumerate(queries):
        ...
        time.sleep(0.001) 
    # print(f"===== Scenario {scenario_name}, Trial {trial_num} Completed. =====")
...
if __name__ == "__main__":
    ...
    # Aggregate data for statistical tests
    # We need lists of values for each metric, per scenario, across all trials.
    scenario_metrics = defaultdict(lambda: defaultdict(list)) # scenario_mode -> metric_name -> list_of_values_per_query_per_trial

    for entry in simulation_results_log:
        mode = entry["scenario_mode"]
        
        # Numerical metrics (cost, latency, epsilon used)
        if entry["is_delegated"]: # Only consider delegated queries for cost/latency/epsilon used by remote
            scenario_metrics[mode]["simulated_total_cost"].append(entry["simulated_total_cost"])
            scenario_metrics[mode]["simulated_total_latency"].append(entry["simulated_total_latency"])
            scenario_metrics[mode]["epsilon_used_by_remote"].append(entry["epsilon_used_by_remote"])
        elif mode == SCENARIO_BASELINE_ALWAYS_LOCAL: # Always Local has its own cost/latency profile
             scenario_metrics[mode]["simulated_total_cost"].append(entry["simulated_total_cost"])
             scenario_metrics[mode]["simulated_total_latency"].append(entry["simulated_total_latency"])

        # Categorical metrics (Delegation decision)
        scenario_metrics[mode]["is_delegated_bool"].append(entry["is_delegated"])
        
        # Response Quality (categorical counts for chi-squared)
        scenario_metrics[mode]["response_quality_counts"].append(entry["response_quality_type"])


    # --- Table 1: ANOVA Results for Average Delegated Cost ---
    print("\n\nTable 1: ANOVA Test Results for Average Simulated Total Cost (Delegated Queries)")
    print("-" * 80)
    
    # Collect cost data for ANOVA (only delegated queries, filter out AlwaysLocal as it doesn't delegate)
    cost_data_for_anova = {
        mode: metrics["simulated_total_cost"] for mode, metrics in scenario_metrics.items()
        if mode != SCENARIO_BASELINE_ALWAYS_LOCAL and metrics["simulated_total_cost"] # Ensure list is not empty
    }
    ...
    # --- Table 2: Chi-Squared Test Results for Delegation Rate ---
    ...
    # We compare two major competing scenarios directly.
    
    # Get counts for AEPB_Dynamic_High_Privacy
    aepb_dynamic_high_delegated = sum(1 for e in scenario_metrics[SCENARIO_AEPB_DYNAMIC_HIGH_PRIVACY]["is_delegated_bool"] if e)
    aepb_dynamic_high_local = sum(1 for e in scenario_metrics[SCENARIO_AEPB_DYNAMIC_HIGH_PRIVACY]["is_delegated_bool"] if not e)
    
    # Get counts for Baseline_AlwaysDelegate
    baseline_always_del_delegated = sum(1 for e in scenario_metrics[SCENARIO_BASELINE_ALWAYS_DELEGATE]["is_delegated_bool"] if e)
    baseline_always_del_local = sum(1 for e in scenario_metrics[SCENARIO_BASELINE_ALWAYS_DELEGATE]["is_delegated_bool"] if not e)

    # Create contingency table: [[Delegated, Not Delegated], [Delegated, Not Delegated]]
    ...
    if (aepb_dynamic_high_delegated + aepb_dynamic_high_local > 0 and 
        baseline_always_del_delegated + baseline_always_del_local > 0): # Ensure valid data for test
        ...
        print("Not enough data to perform Chi-squared test for Delegation Rate.")
    print("-" * 80)
