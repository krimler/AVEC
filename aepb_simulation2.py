...
def get_ppo_sensitivity_score(query: str) -> float:
    ...
    medium_sensitivity_keywords = [
        ...
        r'\bmy\s+consumption\s+habits\b', r'\bmy\s+property\b', # Environmental (personal environmental impact)
...
def get_local_agent_confidence(query: str, has_local_cache_hit: bool = False, local_model_capability: str = "basic_qa") -> float:
    ...
    base_confidence = random.uniform(0.3, 0.6) # More variability compared to 0.4 fixed
    ...
    if len(query) < 40 and not any(k in query_lower for k in ["medical", "finance", "acquisition", "history", "private jet", "legal", "patent", "my", "personal", "confidential", "secret", "proprietary"]): # Added more general sensitive terms
        base_confidence += random.uniform(0.1, 0.3) # Added variability
        
    if has_local_cache_hit:
        base_confidence = random.uniform(0.85, 0.98) # High confidence if from a trusted cache, but still some range
    ...
    base_confidence -= (ppo_sens * random.uniform(0.2, 0.4)) # More sensitive = less confident locally, with variability
...
class TranslationAgent:
    def transform_request(self, original_query: str, epsilon_budget: float) -> (str, dict, float, float):
        # Simulate cost/latency of Translation Agent's work with more variability
        simulated_latency = random.uniform(0.01, 0.03) + len(original_query) * random.uniform(0.00005, 0.00015)
        simulated_cost = random.uniform(0.0005, 0.0015) + len(original_query) * random.uniform(0.000005, 0.000015)
...
def run_simulation_scenario(client_instance: Client, queries: list, scenario_name: str, trial_num: int, fixed_epsilon_value: float = None):
    ...
    # Randomize prompt order for this user in this trial (added in previous step)
...
if __name__ == "__main__":
    ...
    # Use total counts across all queries and trials.
