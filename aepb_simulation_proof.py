...
class TranslationAgent:
    def transform_request(self, original_query: str, epsilon_budget: float) -> (str, dict, float, float):
        ...
        simulated_latency = random.uniform(0.003, 0.07) + len(original_query) * random.uniform(0.00002, 0.0003) # Wider range for base and length multiplier
        simulated_cost = random.uniform(0.0001, 0.0035) + len(original_query) * random.uniform(0.000002, 0.00003) # Wider range for base and length multiplier
...
class RemoteAgent:
    def process_request(self, privacy_enhanced_request: str, epsilon_budget: float) -> (str, float, float):
        ...
        simulated_latency = random.uniform(0.3, 0.8) + len(privacy_enhanced_request) * random.uniform(0.00003, 0.00008) # Wider range for base and length multiplier
        simulated_cost = random.uniform(0.007, 0.015) + len(privacy_enhanced_request) * random.uniform(0.000007, 0.000015) # Wider range for base and length multiplier
        ...
        if "Highly Privatized" in privacy_enhanced_request:
            ...
            simulated_cost *= random.uniform(0.3, 0.7) # Cheaper for more privacy, with wider variability
        elif "Moderately Privatized" in privacy_enhanced_request:
            response_content = " (General information provided with some details.)"
            simulated_cost *= random.uniform(0.6, 0.95) # Wider range
            ...
            simulated_cost *= random.uniform(0.85, 1.15) # Wider range
            ...
            simulated_cost *= random.uniform(0.9, 1.1) # Wider range
...
if __name__ == "__main__":
    ...
    # This test compares two specific groups: AEPB_Dynamic_High_Privacy vs. Baseline_AlwaysDelegate
    print("\n\nTable 2: Mann-Whitney U-test Results for Delegation Rate (AEPB HighP vs AlwaysDel)")
    print("-" * 80)
    ...
    for trial_num in range(1, NUM_TRIALS + 1):
        # Ensure we filter by trial_num for the specific scenario
        ...
        else: # If a trial generated no data for this scenario, ensure it's still accounted for to match NUM_TRIALS
            aepb_high_p_delegation_rates.append(0.0) # Or NaN, depending on how you want to handle missing trials
    ...
    for trial_num in range(1, NUM_TRIALS + 1):
        ...
        if not trial_df.empty:
            ...
            baseline_always_del_delegation_rates.append(1.0) # AlwaysDelegate implies 100% delegation if any query ran

    if aepb_high_p_delegation_rates and baseline_always_del_delegation_rates:
        # Check if lists have same length and contain enough data points for Mann-Whitney
        if len(aepb_high_p_delegation_rates) >= 2 and len(baseline_always_del_delegation_rates) >= 2:
            ...
            print("Not enough valid *per-trial* data points to perform Mann-Whitney U-test for Delegation Rate (min 2 per group).")
            print(f"AEPB HighP rates count: {len(aepb_high_p_delegation_rates)}, AlwaysDel rates count: {len(baseline_always_del_delegation_rates)}")
    else:
